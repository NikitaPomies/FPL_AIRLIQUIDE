...
function swap!(new_sol::Solution, s1::Int64,s2::Int64)
    ...
    if sum(new_sol.isproduction)>1
        ...
        new_sol.isdistribution[s2]=new_sol.isproduction[s1]
        new_sol.isproduction[s2]=new_sol.isautomated[s1]

        new_sol.isproduction[s1]=temp_prod
        new_sol.isdistribution[s1]=temp_distrib
        new_sol.isproduction[s1]=temp_automated
    end
...
function perturbation(instance::Instance,current_sol::Solution)
    ...
    for s in liste
        liste_func=[create_prodsite!,destroy_site!]
